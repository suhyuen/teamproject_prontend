...
export default function SignupPage() {
  ...
  const handleClickJoinButton = async () => {
    ...
    if (idCheck.current !== "ok") {
      // id 확인
      normalization = false;
      errorMessage = "id 체크를 해주세요";
    } else if (nicknameCheck.current !== "ok") {
      // 닉네임 확인
      normalization = false;
      errorMessage = "닉네임 체크를 해주세요";
    } else if (passwordRegex.test(formData.userPw) === false) {
      // pw 확인1
      normalization = false;
      errorMessage =
      ...
      <div className="lgoinContaner" onClick={handleClickLoginContaner}>
        ...
        <form className="content">
          <div>
            ...
            ></input>
            <button type="button" onClick={handleClickIdButton}>
              중복체크
            </button>
          </div>
          <div>
        ...
        <button className="button" onClick={handleClickJoinButton}>
          회원가입
        </button>
      </div>
      <Footer></Footer>
